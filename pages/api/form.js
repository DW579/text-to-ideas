...
export default async function handler(req, res) {
    if (req.method === "POST") {
        try {
            ...
            const ideasModel = new OpenAI({ temperature: 1 });
            const ideasTemplate = `Create 5 large language modal app ideas to address the issue provided: 
            
            {mainIssue}
            
            Return your response in a json object with the mainIssue and an array of ideas with the title and description of each idea.`;
